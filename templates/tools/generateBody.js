...
async function generateBody(repos) {
	...
	for (const repo in repos) {
		// go over every repo in the sections object
		const heading = `### ${repo}`;
			...
			if (state === 'open') {
				for (const instance in repos[repo][state]) {
					...
					if (
						...
						if (repos[repo][state][instance].type === 'Issue') {
							// if the instance type is an issue, write the the appropriate header
							const localHeading = await generateSectionHeader(
								4,
								'Issues',
								state,
							);
							...
							// if the instance type is a pull request, write the the appropriate header
							const localHeading = await generateSectionHeader(
								4,
								'Pull Requests',
								state,
							...
							// if the instance type is a Discussion, write the the appropriate header
							const localHeading = await generateSectionHeader(
								4,
								'Discussions',
								state,
					...
					markdown = markdown.concat(
						`${repos[repo][state][instance].bullet}\n`,
					);
				}
				...
				for (const instance in repos[repo][state]) {
					...
					if (
						...
						// caching repos[repo][state][instance].type here is important because it ensures that we're getting _out_ all the correct headers
						garbageCache.push(
							`${heading} ${state} ${repos[repo][state][instance].type}`,
						);
						const localHeading = await generateSectionHeader(
				...
				for (const instance in repos[repo][state]) {
					...
					if (
						...
						// same comment here about repos[repo][state][instance].type as above
						garbageCache.push(
							`${heading} ${state} ${repos[repo][state][instance].type}`,
						);
						if (repos[repo][state][instance].type === 'Issue') {
