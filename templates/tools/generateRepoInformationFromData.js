	...
	for (const type in data) {
		if (
			...
			for (const instance of data[type]) {
				...
				if (
					instance.meta.type === 'Discussion' ||
					(instance.status.state === 'OPEN' && !instance.status.locked)
				) {
					repos[instance.repo.name].open.push({
						bullet,
						type: instance.meta.type,
					});
				} else if (instance.status.state === 'MERGED') {
					repos[instance.repo.name].merged.push({
						bullet,
						type: instance.meta.type,
					});
				} else if (
					...
					repos[instance.repo.name].closed.push({
						bullet,
						type: instance.meta.type,
					});
				}
