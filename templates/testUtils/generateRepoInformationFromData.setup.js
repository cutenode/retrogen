...
async function setup() {
	...
	const nodejsReferencePath = resolve(
		`${dir}/testUtils/reference/generateRepoInformationFromData.nodejs.json`,
	);
	const kubernetesReferencePath = resolve(
		`${dir}/testUtils/reference/generateRepoInformationFromData.kubernetes.json`,
	);
	...
	await writeFile(
		nodejsReferencePath,
		JSON.stringify(nodejsComposedData, null, 2),
	);
	await writeFile(
		kubernetesReferencePath,
		JSON.stringify(kubernetesComposedData, null, 2),
	);
}
