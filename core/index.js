...
async function generate(owner, dates = {}, options = {}) {
	...
	const data = {
		meta: {
			// general information that we'll want to use in multiple places
			issues: {
				count: 0,
				authors: {
					count: 0,
	...
	for (const issue in results.issuesPrs.nodes) {
		...
		if (results.issuesPrs.nodes[issue].__typename === 'PullRequest') {
			// increment the count of unique authors
			if (
				!data.meta.pullRequests.authors.list.includes(usable.author.username)
			) {
	...
	for (const discussion in results.discussions.nodes) {
		const usable = await returnUsable(
			results.discussions.nodes[discussion],
			'Discussion',
		);
		...
		if (data.meta.owner === '') {
			data.meta.owner =
				results.discussions.nodes[discussion].repository.owner.login;
		}
